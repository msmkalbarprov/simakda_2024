...
function khusus_spd($params, $jns_ang)
{
    switch ($jns_ang) {
        case 'M':
            ...
            break;
        case 'P1':
            $data = "Peraturan Gubernur Nomor 1 Tahun 2023 tentang Perubahan Atas Peraturan Gubernur Nomor 96 Tahun 2022 tentang Penjabaran Anggaran dan Pendapatan dan Belanja Daerah Tahun Anggaran 2023 tanggal 9 Januari 2023. Berita daerah nomor 1";
            break;
        case 'P2':
...
function sp2dbelanja_barang($kd_skpd, $no_spp, $tgl_spp, $beban, $kd_sub_kegiatan)
{
    if ($beban == '2') {
        ...
        $nilai = DB::table('trdspp as b')
            ...
            ->where(['a.jns_spp' => '6', 'a.kd_skpd' => $kd_skpd, 'b.kd_sub_kegiatan' => $kd_sub_kegiatan])
            ->where('a.no_spp', '!=', $no_spp)
            ->where('c.tgl_sp2d', '<=', $tgl_spp)
            ->where(function ($query) {
                $query->where('a.sp2d_batal', '')->orWhereNull('a.sp2d_batal');
...
function no_urut($kd_skpd)
{
    ...
    if ($kd_skpd == '1.01.2.22.0.00.01.0000' || $kd_skpd == '4.01.0.00.0.00.01.0003' || $kd_skpd == '1.02.0.00.0.00.02.0000' || $kd_skpd == '1.02.0.00.0.00.03.0000') {
        ...
        $urut = collect(DB::select("SELECT  case when max(nomor+1) is null then 1 else max(nomor+1) end as nomor from (
            ...
            select no_kas nomor, 'Drop Uang ke Bidang' ket,kd_skpd_sumber as kd_skpd from tr_setorpelimpahan where  isnumeric(no_kas)=1 UNION ALL
            select no_terima nomor, 'Penerimaan BOS' ket,kd_skpd from tr_terima_bos where  isnumeric(no_terima)=1) z WHERE KD_SKPD = ?", [$kd_skpd]))->first();
        return $urut->nomor;
    } else {
    ...
    if ($beban == '1') {
        ...
        $data = DB::select("SELECT a.kd_rek6,a.nm_rek6,
					...
					SELECT SUM(nilai) FROM
                        ...
                        order by a.kd_rek6", [$beban, $no_bukti, $beban, $no_bukti, $beban, $kd_skpd, $kd_sub_kegiatan, $kd_skpd, $jenis_ang]);
    } else {
        $data = DB::select("SELECT kd_rek6, nm_rek6,
                ...
                FROM
                    ...
                    FROM
                        ...
                        INNER JOIN trhspm c ON b.no_spp= c.no_spp
                        AND b.kd_skpd = c.kd_skpd
                        INNER JOIN trhsp2d d ON c.no_spm= d.no_Spm
                        AND c.kd_skpd= d.kd_skpd
                    WHERE
                        d.no_sp2d = ?
...
function cari_rekening_kkpd($kd_sub_kegiatan, $kd_skpd, $jenis_ang, $beban, $no_bukti, $no_sp2d)
{
    if ($beban == '1') {
        $data = DB::select("SELECT a.kd_rek6,a.nm_rek6,
                    (SELECT SUM(nilai) FROM
					...
					SELECT SUM(nilai) FROM
                        ...
                        and a.status_aktif='1'
                        and jns_ang=?
                        and (left(kd_rek6,2)='52' OR left(kd_rek6,4)='5102' OR left(kd_rek6,6)='510204')
                        order by a.kd_rek6", [$beban, $no_bukti, $beban, $no_bukti, $beban, $kd_skpd, $kd_sub_kegiatan, $kd_skpd, $jenis_ang]);
    }
    return $data;
}
...
function cari_dana($sumber, $kd_sub_kegiatan, $kd_rekening, $kd_skpd, $no_sp2d, $no_spp, $beban)
{
    if ($beban == '1') {
        ...
        $data7 = DB::table('trhtransout_cmsbank as a')->join('trdtransout_cmsbank as b', function ($join) {
        ...
        })->select(DB::raw("'trans cms' as jdl"), DB::raw("ISNULL(SUM(ISNULL(b.nilai,0)),0) as nilai"))->unionAll($data6);

        $data8 = DB::table('trhtransout_kkpd as a')->join('trdtransout_kkpd as b', function ($join) {
            $join->on('a.no_voucher', '=', 'b.no_voucher');
            $join->on('a.kd_skpd', '=', 'b.kd_skpd');
        })->where(['b.kd_skpd' => $kd_skpd, 'b.kd_sub_kegiatan' => $kd_sub_kegiatan, 'b.kd_rek6' => $kd_rekening, 'b.sumber' => $sumber, 'a.no_sp2d' => $no_sp2d])->whereNotNull('a.no_voucher')->where(function ($query) {
            $query->where('status_validasi', '0')->orWhereNull('status_validasi');
...
function sisa_bank()
{
    ...
    if ($kd_skpd == '5.02.0.00.0.00.02.0000') {
        ...
        SUM(case when jns=2 then jumlah else 0 end) AS keluar
        from (
        SELECT tgl_kas AS tgl,no_kas AS bku,keterangan as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM tr_setorsimpanan where (kkpd<>'1' or kkpd is null)
        union all
...
function sisa_bank_kkpd($kd_skpd, $bulan)
{
    ...
    $sisa_kkpd  = collect(DB::select("SELECT terima-keluar as sisa FROM(
                                ...
                                )zz", [$kd_skpd, $bulan, $kd_skpd, $bulan, $kd_skpd, $bulan, $kd_skpd, $bulan]))->first();
...
function sisa_bank_by_bulan($kd_skpd, $bulan)
{
    ...
    if ($kd_skpd == '5.02.0.00.0.00.02.0000') {
        $data = collect(DB::select("SELECT terima-keluar as sisa FROM(select
                ...
                and keterangan not like '%7819/GU/2023%' and (kkpd<>1 OR kkpd is null)
                union
                SELECT tgl_sp2d AS tgl,no_sp2d AS bku,keperluan as ket,
                sum(b.nilai)-(select kkpd from ms_up where kd_skpd=a.kd_skpd)as jumlah,
                '1' AS jns,a.kd_skpd AS kode FROM trhsp2d a inner join trdspp b on a.no_spp=b.no_spp and a.kd_skpd=b.kd_skpd where a.no_sp2d IN (SELECT isnull(no_sp2d,'') FROM up_kkpd where kd_skpd=?) and (b.kkpd!='1' or b.kkpd is null) and a.status='1' and a.jns_spp IN ('1','2') and  a.kd_skpd=? GROUP BY a.tgl_sp2d,a.no_sp2d,a.keperluan,a.kd_skpd
        ...
        $data = collect(DB::select("SELECT terima-keluar as sisa FROM(select
                ...
                SELECT tgl_kas AS tgl,no_kas AS bku,keterangan as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM tr_setorsimpanan where (tunai<>1 OR tunai is null) and (kkpd<>1 OR kkpd is null)
                union
                SELECT tgl_bukti AS tgl,no_bukti AS bku,ket as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM trhINlain WHERE pay='BANK'
                union
                select c.tgl_kas [tgl],c.no_kas [bku] ,c.keterangan [ket],c.nilai [jumlah],'1' [jns],c.kd_skpd [kode] from tr_jpanjar c join tr_panjar d on
                ...
                WHERE pay='BANK' and
                (panjar not in ('1') or panjar is null) and (kkpd is null or kkpd<>'1')
                union
                select a.tgl_bukti [tgl],a.no_bukti [bku],a.ket [ket],sum(b.nilai) [jumlah],'2' [jns],a.kd_skpd [kode] from trhstrpot a
                join trdstrpot b on a.no_bukti=b.no_bukti and a.kd_skpd=b.kd_skpd
                where a.kd_skpd=? and a.pay='BANK' group by a.tgl_bukti,a.no_bukti,a.ket,a.kd_skpd
...
// HELPER SUMBER DANA
function sumber_dana($no_trdrka, $status_anggaran)
{
...
function angkas1($sts_angkas, $kode, $giat, $rek, $bulan, $status_anggaran)
{
    if ($sts_angkas == 'murni') {
        $field_angkas = 'nilai_susun';
    } else if ($sts_angkas == 'murni_geser1') {
...
function angkas_lalu_penagihan($kode, $giat, $rek)
{
    $data =
        collect(DB::select("SELECT SUM(nilai) total FROM
                                    (
                                         --Table tampungan
...
function spd_penagihan($kode, $giat, $rek, $revisi1, $revisi2, $revisi3, $revisi4)
{
    $data =
...
function sisa_bank_kkpd1()
{
    ...
    FROM (
            ...
            -- INNER JOIN up_kkpd b on a.kd_skpd=b.kd_skpd and a.no_sp2d=b.no_sp2d
            INNER JOIN trdspp c on a.no_spp=c.no_spp and a.kd_skpd=c.kd_skpd
            where a.jns_spp='2' and a.kd_skpd= ? and c.kkpd=1 and a.status=1
            ...
            SELECT isnull(SUM(a.nilai),0) as jumlah,'2' as jns FROM trdtransout a INNER JOIN trhtransout b
            ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd
            WHERE b.kd_skpd= ? and b.kkpd=1
            UNION ALL
            -- dropping
            SELECT isnull(sum(nilai),0),'2' as jns from tr_setorpelimpahan_bank_cms
            where kd_skpd_sumber= ? and status_validasi = 1 and kkpd=1
            UNION ALL
            SELECT isnull(sum(nilai),0),'1' as jns from tr_setorsimpanan
            where kd_skpd= ? and kkpd=1

            )z", [$kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd]))->first();

    return $sisa_kkpd;
}

function sisa_bank_kkpd2()
{
    $kd_skpd = Auth::user()->kd_skpd;
