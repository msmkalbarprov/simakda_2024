...
class KoreksiDataController extends Controller
{
    ...
    public function sp2d(Request $request)
    {
        ...
        $data_spm = DB::select("SELECT a.no_spm,a.keperluan,b.tgl_mulai,b.tgl_akhir,(SELECT jenis FROM trhtagih c WHERE b.no_tagih=c.no_bukti AND b.kd_skpd=c.kd_skpd) as jenis FROM trhspm a INNER JOIN trhspp b ON a.no_spp=b.no_spp AND a.kd_skpd=b.kd_skpd WHERE a.kd_skpd=? AND (b.sp2d_batal!=? OR b.sp2d_batal is null) AND no_spm not in (SELECT no_spm,status_bud from trhsp2d where status_bud = 1 and (sp2d_batal!= 1 OR sp2d_batal is null))", [$kd_skpd, '1']);
