...
class PenyetoranController extends Controller
{
    ...
    public function nomorPenyetoranIni(Request $request)
    {
        ...
        $data = DB::table('tr_terima as a')
            ...
            ->whereRaw("a.kd_skpd=? AND a.no_terima+'.'+kanal NOT IN(select ISNULL(no_terima,'')+'.'+kanal no_terima from trdkasin_pkd where kd_skpd=?) AND  a.tgl_terima=?", [$kd_skpd, $kd_skpd, $tgl_terima])
