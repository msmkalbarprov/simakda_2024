...
class TransaksiKKPDController extends Controller
{
    ...
    public function loadData()
    {
        ...
        $data = DB::table('trhtransout_kkpd as a')
            ->select('a.*', DB::raw("'' as nokas_pot"), DB::raw("'' as tgl_pot"), DB::raw("'' as kete"))
            ->where(['a.panjar' => '0', 'kd_skpd' => $kd_skpd])
            ->orderBy('tgl_voucher')
    ...
    public function cariRekening(Request $request)
    {
        ...
        $rekening = cari_rekening_kkpd($kd_sub_kegiatan, $kd_skpd, $jenis_ang, $beban, $no_bukti, $no_sp2d);
    ...
    public function cariSumber(Request $request)
    {
        ...
        if ($beban == '1') {
            ...
            $sumber = DB::table(DB::raw("({$sumber2->toSql()}) AS sub"))
                ->mergeBindings($sumber2)
                ->get();
        } else {
            $sumber = DB::table('trhspp as a')
                ...
                ->where(['c.no_sp2d' => $no_sp2d, 'a.kd_skpd' => $kd_skpd, 'b.kd_sub_kegiatan' => $kd_sub_kegiatan, 'kd_rek6' => $kd_rek6])->groupBy('b.sumber')
        ...
        if ($beban == '4' || substr($kd_sub_kegiatan, 5, 10) == '01.1.02.01') {
            ...
            })->where(['a.kd_skpd' => $kd_skpd, 'a.kd_sub_kegiatan' => $kd_sub_kegiatan, 'kd_rek6' => $kd_rek6, 'jns_ang' => $jenis_ang])->where('bulan', '<=', $bulan1)->groupBy('a.kd_sub_kegiatan', 'a.kd_rek6')->select('a.kd_sub_kegiatan', DB::raw("SUM(a.$angkas) as nilai"))->first();
        } else {
            $angkas = DB::table('trdskpd_ro as a')->join('trskpd as b', function ($join) {
                $join->on('a.kd_skpd', '=', 'b.kd_skpd');
                $join->on('a.kd_sub_kegiatan', '=', 'b.kd_sub_kegiatan');
            })->where(['a.kd_skpd' => $kd_skpd, 'a.kd_sub_kegiatan' => $kd_sub_kegiatan, 'kd_rek6' => $kd_rek6, 'jns_ang' => $jenis_ang])->where('bulan', '<=', $bulan)->groupBy('a.kd_sub_kegiatan', 'a.kd_rek6')->select('a.kd_sub_kegiatan', DB::raw("SUM(a.$angkas) as nilai"))->first();
        }

        // ANGKAS LALU
        if ($beban == '1') {
            $data1 = DB::table('trdtransout as c')
                ->join('trhtransout as d', function ($join) {
                    $join->on('c.no_bukti', '=', 'd.no_bukti');
                    $join->on('c.kd_skpd', '=', 'd.kd_skpd');
                })
                ->select(DB::raw("SUM(ISNULL(c.nilai,0)) as nilai"))
                ->where(['c.kd_sub_kegiatan' => $kd_sub_kegiatan, 'd.kd_skpd' => $kd_skpd, 'c.kd_rek6' => $kd_rek6, 'd.jns_spp' => '1']);

            $data2 = DB::table('trdtransout_cmsbank as c')
            ...
            $data3 = DB::table('trdspp as c')
                ->join('trhspp as d', function ($join) {
                    $join->on('c.no_spp', '=', 'd.no_spp');
                    $join->on('c.kd_skpd', '=', 'd.kd_skpd');
                })
                ->select(DB::raw("SUM(ISNULL(c.nilai,0)) as nilai"))
                ->where(['c.kd_sub_kegiatan' => $kd_sub_kegiatan, 'c.kd_skpd' => $kd_skpd, 'c.kd_rek6' => $kd_rek6])->whereIn('d.jns_spp', ['3', '4', '5', '6'])
                ->where(function ($query) {
                    $query->where('sp2d_batal', '')->orWhereNull('sp2d_batal');
                })
                ->unionAll($data2);

            $data4 = DB::table('trdtagih as c')
                ->join('trhtagih as d', function ($join) {
                    $join->on('c.no_bukti', '=', 'd.no_bukti');
                    $join->on('c.kd_skpd', '=', 'd.kd_skpd');
                })
                ->select(DB::raw("SUM(ISNULL(nilai,0)) as nilai"))
                ->where(['c.kd_sub_kegiatan' => $kd_sub_kegiatan, 'd.kd_skpd' => $kd_skpd, 'c.kd_rek6' => $kd_rek6])
                ->whereRaw('d.no_bukti NOT IN (SELECT no_tagih FROM trhspp WHERE kd_skpd=?)', [$kd_skpd])
                ->unionAll($data3);

            $data5 = DB::table('trdtransout_kkpd as c')
            ...
            $data3 = DB::table('trdspp as c')
                ...
                ->select(DB::raw("SUM(ISNULL(c.nilai,0)) as nilai"))
                ->where(['c.kd_sub_kegiatan' => $kd_sub_kegiatan, 'c.kd_skpd' => $kd_skpd, 'c.kd_rek6' => $kd_rek6])
                ->whereIn('d.jns_spp', ['3', '4', '5', '6'])
                ->where('d.no_spp', '<>', $no_spp)
                ->where(function ($query) {
                    $query->where('sp2d_batal', '')->orWhereNull('sp2d_batal');
                })
                ->unionAll($data2);
    ...
    public function simpanCms(Request $request)
    {
        ...
        try {
            $cek = DB::table('trhtransout_kkpd')
                ->where(['no_voucher' => $data['no_bukti'], 'kd_skpd' => $data['kd_skpd']])
                ->count();
            ...
            if (isset($data['rincian_rekening'])) {
                DB::table('trdtransout_kkpd')->insert(array_map(function ($value) use ($data) {
                    return [
                        'no_voucher' => $data['no_bukti'],
    ...
    {
        $kd_skpd = Auth::user()->kd_skpd;
        $data = DB::table('trhtrmpot_kkpd as a')
            ->select('a.*')
            ->selectRaw("(SELECT status_upload FROM trhtransout_kkpd WHERE no_voucher=a.no_voucher and kd_skpd=?)  as status_upload", [$kd_skpd])
            ->selectRaw("(SELECT status_validasi FROM trhtransout_kkpd WHERE no_voucher=a.no_voucher and kd_skpd=?)  as status_validasi", [$kd_skpd])
        ...
        return DataTables::of($data)->addIndexColumn()->addColumn('aksi', function ($row) {
            if ($row->status_validasi != '1') {
                $btn = '<a href="' . route("skpd.transaksi_kkpd.edit_potongan", Crypt::encryptString($row->no_bukti)) . '" class="btn btn-warning btn-sm" style="margin-right:4px"><i class="uil-edit"></i></a>';
                $btn .= '<a href="javascript:void(0);" onclick="hapusPotongan(' . $row->no_bukti . ',\'' . $row->no_voucher . '\');" class="btn btn-danger btn-sm"><i class="uil-trash"></i></a>';
            } else {
                $btn = '';
            }
            return $btn;
    ...
    public function createPotongan()
    {
        ...
        $potongan1 = DB::table('trhtransout_kkpd as a')
            ->join('trdtransout_kkpd as b', function ($join) {
                $join->on('a.no_voucher', '=', 'b.no_voucher');
                $join->on('a.kd_skpd', '=', 'b.kd_skpd');
            })
            ->where(['a.kd_skpd' => $kd_skpd, 'a.status_verifikasi' => '1'])
            ->whereIn('a.jns_spp', ['1', '2', '3'])
            ->where('a.status_upload', '!=', '1')
            ->whereRaw("a.no_voucher NOT IN (SELECT no_voucher FROM trhtrmpot_kkpd a WHERE a.kd_skpd=?)", $kd_skpd)
            ->select('a.no_tgl', 'a.no_voucher', 'a.tgl_voucher', 'b.no_sp2d', 'b.kd_sub_kegiatan', 'b.nm_sub_kegiatan', 'b.kd_rek6', 'b.nm_rek6', 'a.jns_spp', 'a.total', 'a.potongan')
            ->distinct();
    ...
    public function simpanPotongan(Request $request)
    {
        ...
        try {
            ...
            // TRHTRMPOT
            DB::table('trhtrmpot_kkpd')
                ->where(['kd_skpd' => $data['kd_skpd'], 'no_bukti' => $no_bukti])
                ->delete();
            ...
            DB::table('trhtransout_kkpd')
                ->where(['kd_skpd' => $data['kd_skpd'], 'no_voucher' => $data['no_transaksi']])
                ->update([
                    'status_trmpot' => '1',
                ]);

            // TRDTRMPOT
            DB::table('trdtrmpot_kkpd')
