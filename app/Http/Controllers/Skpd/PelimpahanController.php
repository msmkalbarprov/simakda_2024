...
class PelimpahanController extends Controller
{
    ...
    public function prosesUpload(Request $request)
    {
        ...
        try {
            ...
            $no_upload2 = DB::table('trdupload_cmsbank_bidang as a')->leftJoin('trhupload_cmsbank_bidang as b', function ($join) {
                $join->on('a.kd_skpd', '=', 'b.kd_skpd');
                $join->on('a.no_upload', '=', 'b.no_upload');
            })->select('a.no_upload_tgl as nomor', 'b.tgl_upload as tanggal', DB::raw("'Urut Upload Setor Dropping Bank cms' as ket"), 'a.kd_skpd')->where(['a.kd_skpd' => $kd_skpd]);
    ...
    public function draftValidasi()
    {
        ...
        })->where(['a.kd_skpd_sumber' => $kd_skpd, 'a.status_upload' => '1'])->orderBy(DB::raw("CAST(a.no_bukti as int)"))->orderBy('a.kd_skpd')->get();
        return Datatables::of($data)->addIndexColumn()->make(true);
        return view('skpd.pelimmpahan.validasi');
    }
