...
class TransaksiCmsController extends Controller
{
    ...
    public function cariSp2d(Request $request)
    {
        ...
        if ($beban == '3') {
            $data = DB::table('trhspp as a')
                ...
                ->where(['c.kd_skpd' => $kd_skpd, 'c.status' => '1'])
                ->whereRaw($where, [$beban, $kd_sub_kegiatan])
                ->whereRaw('c.no_sp2d NOT IN (SELECT no_sp2d FROM trhlpj WHERE kd_skpd=?)', [$kd_skpd])
                ->orderByDesc('c.tgl_sp2d')
                ->orderBy('c.no_sp2d')
                ->select('c.no_sp2d', 'c.tgl_sp2d', 'c.nilai', DB::raw("'0' as sisa"))
                ->distinct()
                ->get();
        } else if ($beban == '6') {
            $data = DB::table('trhspp as a')
            ...
            $data = DB::table('trhspp as a')
                ...
                ->where(function ($query) {
                    $query->where('a.kkpd', '!=', '1')->orWhereNull('a.kkpd');
                })
                ->where(DB::raw("LEFT(c.kd_skpd,17)"), $kode)
                ->whereRaw($where, [$beban, $kd_sub_kegiatan])
                ->orderByDesc('c.tgl_sp2d')
                ->orderBy('c.no_sp2d')
                ->select('c.no_sp2d', 'c.tgl_sp2d', 'c.nilai', DB::raw("'0' as sisa"))
    ...
    public function cariSumber(Request $request)
    {
        ...
        if ($beban == '1') {
            ...
            $data7 = DB::table('trddpr as c')->join('trhdpr as d', function ($join) {
            ...
            })->select(DB::raw("SUM(ISNULL(c.nilai,0)) as nilai"))->unionAll($data6);

            $data8 = DB::table('trddpt as c')->join('trhdpt as d', function ($join) {
            ...
            $angkas_lalu = DB::table(DB::raw("({$data8->toSql()}) AS sub"))
        ...
        if ($beban == '1') {
            if ($kd_skpd == '1') {
                ...
                //     SUM(case when jns=2 then jumlah else 0 end) AS keluar
                //     from (
                //     SELECT tgl_kas AS tgl,no_kas AS bku,keterangan as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM tr_setorsimpanan where (kkpd is null or kkpd<>'1') and no_sp2d not in (select no_sp2d from  trhsp2d a
                ...
                $sisa_bank = collect(DB::select("SELECT terima,keluar from(SELECT
                    SUM(case when jns=1 then jumlah else 0 end) AS terima,
                    SUM(case when jns=2 then jumlah else 0 end) AS keluar
                    from (
        ...
        // $data_sp2d = DB::select("SELECT kd_rek6, nm_rek6,
        //         (
        //         SELECT SUM
        //             ( nilai )
        //         FROM
        ...
        //             b.kd_rek6,
        //         b.nm_rek6
        //         ) x", [$no_sp2d, $beban, $no_sp2d, $no_bukti, $beban, $no_sp2d, $no_bukti, $beban, $no_sp2d, $no_sp2d, $kd_sub_kegiatan, $kd_rek6]);
    ...
    public function sisaBank(Request $request)
    {
        ...
        if ($beban == '1') {
            if ($kd_skpd == '1') {
                ...
                $data = collect(DB::select("SELECT terima,keluar from(SELECT
                    SUM(case when jns=1 then jumlah else 0 end) AS terima,
                    SUM(case when jns=2 then jumlah else 0 end) AS keluar
                    from (
                    SELECT tgl_kas AS tgl,no_kas AS bku,keterangan as ket,nilai AS jumlah,'1' AS jns,kd_skpd AS kode FROM tr_setorsimpanan where no_sp2d not in (select no_sp2d from  trhsp2d a
    ...
    public function cetakList(Request $request)
    {
        ...
        where month(tgl)<=? and kode=?", [$kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, $kd_skpd, '9', $kd_skpd]))->first();
