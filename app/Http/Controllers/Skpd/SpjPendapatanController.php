...
class SpjPendapatanController extends Controller
{
    ...
    public function cetakSpjPendapatan(Request $request)
    {
         ...
         from ( 
            ...
            FROM trdrka_pend z WHERE left(z.kd_skpd,len(?))=?  and z.jns_ang=?
         ...
         from ( 
            ...
            FROM trdrka_pend z WHERE left(z.kd_skpd,len(?))=?  and z.jns_ang=?
            and left(z.kd_rek6,1)='4' and right(z.kd_sub_kegiatan,5)='00.04'
            GROUP BY z.kd_skpd, z.kd_sub_kegiatan, left(z.kd_rek6,2))zzz group by kd_skpd,left(kd_skpd,len(?)),kd_sub_kegiatan,kd_rek )a 
            left join (select kd_rek2, nm_rek2 from ms_rek2) b on a.kd_rek=b.kd_rek2 
         ...
         from ( 
            ...
            (SELECT isnull(SUM(case when jns_trans in ('3') then b.rupiah*-1 else b.rupiah end),0) FROM trhkasin_pkd a INNER JOIN trdkasin_pkd b 
            ON RTRIM(a.no_sts)=RTRIM(b.no_sts) and a.kd_skpd=b.kd_skpd 
            WHERE left(b.kd_rek6,4)=left(z.kd_rek6,4) and a.kd_skpd=z.kd_skpd and left(b.kd_rek6,12)<>'410411010001' and 
            (a.tgl_sts >= ? AND a.tgl_sts <= ?))
            ...
            FROM trdrka_pend z WHERE left(z.kd_skpd,len(?))=?  and z.jns_ang=?
            and left(z.kd_rek6,1)='4' and right(z.kd_sub_kegiatan,5)='00.04'
            GROUP BY z.kd_skpd, z.kd_sub_kegiatan, left(z.kd_rek6,4))zzz group by kd_skpd,left(kd_skpd,len(?)),kd_sub_kegiatan,kd_rek )a 
            left join (select kd_rek3, nm_rek3 from ms_rek3) b on a.kd_rek=b.kd_rek3 
         ...
         from ( 
            ...
            (SELECT isnull(SUM(case when jns_trans in ('3') then b.rupiah*-1 else b.rupiah end),0) FROM trhkasin_pkd a INNER JOIN trdkasin_pkd b 
            ON RTRIM(a.no_sts)=RTRIM(b.no_sts) and a.kd_skpd=b.kd_skpd 
            WHERE left(b.kd_rek6,6)=left(z.kd_rek6,6) and a.kd_skpd=z.kd_skpd and left(b.kd_rek6,12)<>'410411010001' and 
            (a.tgl_sts >= ? AND a.tgl_sts <= ?))
            ...
            FROM trdrka_pend z WHERE left(z.kd_skpd,len(?))=?  and z.jns_ang=?
            and left(z.kd_rek6,1)='4' and right(z.kd_sub_kegiatan,5)='00.04'
            GROUP BY z.kd_skpd, z.kd_sub_kegiatan, left(z.kd_rek6,6))zzz group by kd_skpd,left(kd_skpd,len(?)),kd_sub_kegiatan,kd_rek )a 
            left join (select kd_rek4, nm_rek4 from ms_rek4) b on a.kd_rek=b.kd_rek4 
