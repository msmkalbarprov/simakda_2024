...
class cetakkapitController extends Controller
{
    public function cetak_kapitalisasi(Request $request){
        ...
        $total_real = collect(DB::select("SELECT sum(ang_peg) ang_peg, sum(real_peg) real_peg, sum(ang_brg) ang_brg,sum(real_brg) real_brg, sum(ang_mod) ang_mod, 
                                            sum(real_mod) real_mod, sum(ang_x) ang_x, sum(real_x) real_x, sum(ang_peng) ang_peng, sum(real_peng) real_peng, sum(nil_kap) nil_kap, 
                                            sum(ang_pengx) ang_pengx, sum(real_pengx) real_pengx, sum(ang_kap) ang_kap, 
                                            sum(tot_kap) tot_kap from cetak_kapitalisasi_brewok ('$kd_skpd') where kd_barang!='' "))->first();
        ...
        if ($cetak == '1') {
            ...
            return $pdf->stream('Kapitalisasi.pdf');
        } else {

            header("Cache-Control: no-cache, no-store, must_revalidate");
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachement; filename="Kapitalisasi.xls"');
            return $view;
        }
    }
}
