...
class lampneracaController extends Controller
{
    ...
    public function load_input_lamp_neraca(Request $request)
    {
        ...
        $bulan   = $request->bulan;
        // dd($id);
        $data = DB::select("SELECT no_lamp, kd_rek3, nm_rek3, a.kd_rek5, nm_rek5, kd_rek6, nm_rek6, tahun, merk, no_polisi, fungsi, hukum, lokasi, alamat, sert, luas, satuan, harga_satuan, piutang_awal, piutang_koreksi, piutang_sudah, investasi_awal, sal_awal, kurang, tambah, tahun_n, akhir, kondisi_b, kondisi_rr, kondisi_rb, REPLACE(convert(varchar(max),keterangan),'\n',' ') keterangan, kd_skpd, jumlah, kepemilikan, rincian_beban, jenis_aset, realisasi_janji, nama_perusahaan, no_polis, tgl_awal, tgl_akhir, jam, bulan, masa, tmasa, korplus, kormin, akum_penyu, sisa_umur, status, akum_penyub, kondisi_x, nil_kurang_excomp, status_extracomp, sekolah, kd_rek7, nm_rek7, case when b.kd_rek5 is null then 0 else 1 end jenis from lamp_aset a left join (select distinct kd_rek5 from ms_rek6) b on a.kd_rek5=b.kd_rek5 where kd_skpd='$kd_skpd'");
