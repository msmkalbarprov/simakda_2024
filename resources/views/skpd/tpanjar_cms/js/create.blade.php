<script>
    $(document).ready(function() {
        ...
        //  Cari Sub Kegiatan
        $.ajax({
                url: "{{ route('tpanjar_cms.kegiatan') }}",
                type: "POST",
                dataType: 'json',
                success: function(data) {
            ...
            $('#kd_sub_kegiatan').on('change', function() {
             ...
             //  Cari No Panjar
        $.ajax({
                url: "{{ route('tpanjar_cms.no_panjar') }}",
                type: "POST",
                dataType: 'json',
                success: function(data) {
                    $('#no_panjar').empty();
                    ...
                    $.each(data, function(index, data) {
                        $('#no_panjar').append(
                            `<option value=
                            "${data.no_panjar}" data-nilai="${data.nilai}">
                             ${data.no_panjar} | ${data.nilai}
                             </option>`
                        );
                    })
                }
            ...
            // Oncahnge no panjar
        $('#no_panjar').on('change', function() {
                $('#nilaipanjar').val('');
                let nilai = parseFloat($(this).find(':selected').data('nilai')) || 0;

                $("#nilaipanjar").val(new Intl.NumberFormat('id-ID', {
                minimumFractionDigits: 2
                }).format(nilai));
        });
        ...
        $('#simpan_panjar').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('tpanjar_cms.cek_simpan') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    no_bukti: no_bukti,
        ...
        function simpan_panjar(data) {
            $.ajax({
                url: "{{ route('tpanjar_cms.simpan_panjar') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    data: data,
        ...
        function simpan_detail_panjar(data) {
            $.ajax({
                url: "{{ route('tpanjar_cms.simpan_detail_panjar') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    data: data,
