...
<script>
    $(document).ready(function() {
        ...
        $('#upload_setor').DataTable({
            ...
            lengthMenu: [5, 10],
            ajax: {
                "url": "{{ route('skpd.upload_setor.load_data') }}",
                "type": "POST",
            },
            ...
            columns: [{
                ...
                },
                {
                    ...
                    className: 'text-center',
                    render: function(data, type, row, meta) {
                        return `<button type="button" onclick="lihatData('${data.no_kas}','${data.tgl_kas}','${data.no_sp2d}','${data.keterangan}','${data.kd_sub_kegiatan}','${data.nm_sub_kegiatan}','${data.kd_skpd}')" class="btn btn-info btn-sm"><i class="fas fa-info-circle"></i></button>`
                    }
                },
                {
                    data: 'rekening_awal',
        ...
        $('#draft_upload').DataTable({
            ...
            lengthMenu: [5, 10],
            ajax: {
                "url": "{{ route('skpd.upload_setor.draft_upload') }}",
                "type": "POST",
            },
            createdRow: function(row, data, index) {
                if (data.status_upload == "1" && data.status_validasi == "1") {
        ...
        $('#data_transaksi1').DataTable({
            responsive: true,
            serverSide: true,
            processing: true,
            ajax: {
                "url": "{{ route('skpd.upload_setor.data_upload') }}",
                "type": "POST",
                "data": function(d) {
        ...
        $('#cetakCsv').on('click', function() {
            ...
            let url = new URL("{{ route('skpd.upload_setor.cetak_csv') }}");
    ...
    function batalUpload(no_upload, tgl_upload, no_upload_tgl, kd_skpd) {
        ...
        if (tanya == true) {
            $.ajax({
                url: "{{ route('skpd.upload_setor.hapus') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    no_upload: no_upload,
                ...
                success: function(data) {
                    if (data.message == '1') {
                        ...
                        window.location.href = "{{ route('skpd.upload_setor.index') }}";
                    } else {
