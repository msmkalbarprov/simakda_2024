<script>
    $(document).ready(function() {
        ...
        $('#data_upload').on('select2:select', function() {
            let no_bukti = this.value;
            let total_upload = angka(document.getElementById('total_upload').value);
            ...
            $('#total_upload').val(new Intl.NumberFormat('id-ID', {
                minimumFractionDigits: 2
            }).format(total_upload + total));
            $("#data_upload").val(null).change();
        });
        ...
        $('#proses_upload').on('click', function() {
            ...
            if (tanya == true) {
                ...
                $.ajax({
                    url: "{{ route('skpd.upload_setor.simpan') }}",
                    type: "POST",
                    dataType: 'json',
                    data: {
                    ...
                    success: function(data) {
                        if (data.message == '1') {
                            alert('Data berhasil diupload dengan Nomor Upload : ' + data
                                .no_upload);
                            window.location.href =
                                "{{ route('skpd.upload_setor.index') }}";
                        } else {
    ...
    function deleteData(no_bukti, total) {
        ...
        if (tanya == true) {
            ...
            $('#total_upload').val(new Intl.NumberFormat('id-ID', {
                minimumFractionDigits: 2
            }).format(total_upload - parseFloat(total)));
        } else {
            return false;
        }
    }
