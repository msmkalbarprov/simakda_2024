<script>
    $(document).ready(function() {
        ...
        let nilai_panjar = $('#no_panjar').find(':selected').data('nilai');
        ...
        let sub_kegiatan = "{{ $panjar->kd_sub_kegiatan }}";
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, data) {
                        if (data.kd_sub_kegiatan = sub_kegiatan) {
                            $('#kd_sub_kegiatan').append(
                                `<option selected value="${data.kd_sub_kegiatan}" data-anggaran="${data.anggaran}" data-transaksi="${data.transaksi}">${data.kd_sub_kegiatan} | ${data.nm_sub_kegiatan} | ${new Intl.NumberFormat('id-ID', {minimumFractionDigits: 2}).format(data.transaksi)} | ${new Intl.NumberFormat('id-ID', {minimumFractionDigits: 2}).format(data.anggaran)}</option>`
                            );
        ...
        $('#simpan').on('click', function() {
            ...
            let nilai_simpan = angka(document.getElementById('nilai_simpan').value);
            let pajak = rupiah(document.getElementById('pajak').value);
            ...
            if (sisa_bank + nilai_simpan < nilai && sisa_anggaran + nilai_simpan < nilai) {
            ...
            $.ajax({
                url: "{{ route('tambah_panjarcms.update') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    data: data
