<script>
    $(document).ready(function() {
        ...
        $('#list_blm_validasi').DataTable({
            ...
            lengthMenu: [5, 10],
            ajax: {
                "url": "{{ route('skpd.validasi_setor.load_data') }}",
                "type": "POST",
            },
            columns: [{
                    data: 'DT_RowIndex',
        ...
        $('#draft_validasi').DataTable({
            ...
            lengthMenu: [5, 10],
            ajax: {
                "url": "{{ route('skpd.validasi_setor.draft_validasi') }}",
                "type": "POST",
            },
            columns: [{
                    data: 'DT_RowIndex',
    ...
    function batalValidasi(no_kas, kd_skpd, no_bukti) {
        ...
        if (tanya == true) {
            $.ajax({
                url: "{{ route('skpd.validasi_setor.hapus') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    no_kas: no_kas,
                ...
                success: function(data) {
                    if (data.message == '1') {
                        ...
                        window.location.href = "{{ route('skpd.validasi_setor.index') }}";
                    } else {
