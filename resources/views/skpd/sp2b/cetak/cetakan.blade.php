...
<body>
    ...
    <div style="text-align: center">
        <table style="width: 100%">
            <tr>
                <td><u><b>SURAT PERMINTAAN PENGESAHAN BELANJA (SP2B)</b></u></td>
            </tr>
            <tr>
                <td>
                    <b>Tanggal {{ tanggal($sp2b->tgl_sp2b) }} / Nomor {{ $sp2b->no_sp2b }}</b>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table style="border-collapse:collapse;font-family: Open Sans;width:100%" border="1">
        <tr>
            <td colspan="2" style="text-align: center">PENERIMAAN</td>
            <td colspan="2" style="text-align: center">BELANJA</td>
            <td colspan="2" style="text-align: center">PENGEMBALIAN</td>
        </tr>
        <tr>
        ...
        @foreach ($detail as $item)
            @switch($item->urut)
                @case(1)
                    <tr>
                        <td>{{ $item->kd_rek6 }}</td>
                        <td class="angka">{{ rupiah($item->terima) }}</td>
                        <td>-</td>
                        <td>-</td>
                        <td>-</td>
                        <td>-</td>
                ...
                @case(2)
                    <tr>
                        ...
                        <td>-</td>
                        <td>{{ $item->kd_rek6 }}</td>
                        <td class="angka">{{ rupiah($item->belanja) }}</td>
                        <td>-</td>
                        <td>-</td>
                    </tr>
                @break
                ...
                @case(3)
                    <tr>
                        ...
                        <td>-</td>
                        <td>{{ $item->kd_rek6 }}</td>
                        <td class="angka">{{ rupiah($item->kembali) }}</td>
                    </tr>
                @break
        ...
        @endforeach
        <tr>
            <td style="border-right: hidden">Jumlah Penerimaan </td>
            <td class="angka">{{ rupiah($total_detail->terima) }}</td>
            <td style="border-right: hidden">Jumlah Belanja</td>
            <td class="angka">{{ rupiah($total_detail->belanja) }}</td>
            <td style="border-right: hidden">Jumlah Pengembalian</td>
            <td class="angka">{{ rupiah($total_detail->kembali) }}</td>
        </tr>
    </table>
        ...
        <table class="table" style="width: 100%">
            <tr>
                ...
                <td style="margin: 2px 0px;text-align: center">
                    {{ $daerah->daerah }},{{ tanggal($sp2b->tgl_sp2b) }}
                </td>
            </tr>
            <tr>
                <td style="width: 50%"></td>
