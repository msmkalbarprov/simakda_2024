<script>
    $(document).ready(function() {
        ...
        if (document.getElementById('beban').value == '7') {
            $('#rek_pajak').show();
        } else {
            $('#rek_pajak').hide();
        }
        ...
        $('#simpan_pengeluaran').on('click', function() {
            ...
            $('#simpan_pengeluaran').prop('disabled', true);
            $.ajax({
                url: "{{ route('skpd.pengeluaran_lain.update') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    data: data
                ...
                success: function(response) {
                    if (response.message == '1') {
                        ...
                        window.location.href =
                            "{{ route('skpd.pengeluaran_lain.index') }}";
                    } else {
                        alert('Data tidak berhasil ditambahkan!');
                        $('#simpan_pengeluaran').prop('disabled', false);
                        return;
                    }
                }
            })
