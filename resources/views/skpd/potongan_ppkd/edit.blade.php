...
@section('title', 'Edit Data Potongan PPKD | SIMAKDA')
@section('content')
    <div class="row">
        {{-- Input form --}}
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-4">
                            <input class="form-control" type="text" id="no_kas" name="no_kas"
                                value="{{ $potongan->no_kas }}" required readonly>
                        </div>
                            ...
                            <select class="form-control select2-multiple" style="width: 100%" id="no_sts" name="no_sts">
                                ...
                                @foreach ($data_sts as $sts)
                                    <option value="{{ $sts->no_sts }}" data-tgl="{{ $sts->tgl_sts }}"
                                        ...
                                        {{ $potongan->no_sts == $sts->no_sts ? 'selected' : '' }}>
                            ...
                            <select class="form-control select2-multiple" style="width: 100%" id="jenis" name="jenis">
                                ...
                                @foreach ($daftar_jenis as $jenis)
                                    <option value="{{ $jenis->kd_rek6 }}" data-nama="{{ $jenis->nm_rek6 }}"
                                        {{ $potongan->kd_rek6 == $jenis->kd_rek6 ? 'selected' : '' }}>
                            ...
                            <select class="form-control select2-multiple" style="width: 100%" id="pengirim"
                                ...
                                @foreach ($daftar_pengirim as $pengirim)
                                    <option value="{{ $pengirim->kd_pengirim }}" data-nama="{{ $pengirim->nm_pengirim }}"
                                        {{ $potongan->sumber == $pengirim->kd_pengirim ? 'selected' : '' }}>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-10">
                            <textarea class="form-control" style="width: 100%" id="keterangan" name="keterangan">{{ $potongan->keterangan }}</textarea>
                        </div>
                    </div>
                    <!-- SIMPAN -->
...
@endsection
@section('js')
    @include('skpd.potongan_ppkd.js.edit');
@endsection
