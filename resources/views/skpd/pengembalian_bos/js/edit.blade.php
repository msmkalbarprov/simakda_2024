<script>
    $(document).ready(function() {
        ...
        let tgl_kas = "{{ $bos->tgl_sts }}";
        if (tgl_kas) {
            let tanggal = tgl_kas;
            let kd_sub_kegiatan = "{{ $bos->kd_sub_kegiatan }}";
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, data) {
                        if (kd_sub_kegiatan == data.kd_sub_kegiatan) {
                            $('#kd_sub_kegiatan').append(
                                `<option value="${data.kd_sub_kegiatan}" data-nama="${data.nm_sub_kegiatan}" selected>${data.kd_sub_kegiatan} | ${data.nm_sub_kegiatan}</option>`
                            );
                        } else {
                            $('#kd_sub_kegiatan').append(
        ...
        $('#simpan').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('pengembalian_bos.update') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    data: data
