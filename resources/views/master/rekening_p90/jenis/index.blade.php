...
@section('title', 'REKENING JENIS | SIMAKDA')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-rep-plugin">
                        <div class="table-responsive mb-0" data-pattern="priority-columns">
                            <table id="jenis" class="table" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                        <th>Kode Jenis</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">REKENING JENIS</h5>
                        ...
                        <div class="col-md-8">
                            <select name="kode_akun" id="kode_akun" class="form-control select2-modal">
                                ...
                                @foreach ($daftar_akun as $akun)
                                    <option value="{{ $akun->kd_rek2 }}">{{ $akun->kd_rek2 }} - {{ $akun->nm_rek2 }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.select2-modal').select2({
                dropdownParent: $('#modal_jenis .modal-content'),
                theme: 'bootstrap-5'
            });

            let tabel_jenis = $('#jenis').DataTable({
                responsive: true,
                ordering: false,
                serverSide: true,
                processing: true,
        ...
        function deleteData(id) {
            ...
            let tabel = $('#jenis').DataTable();
            if (tanya == true) {
                $.ajax({
                    url: "{{ route('jenis.hapus') }}",
                    type: "POST",
                    dataType: 'json',
                    data: {
                        kode_rekening: id,
