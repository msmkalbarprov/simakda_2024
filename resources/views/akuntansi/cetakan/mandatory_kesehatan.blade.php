                ...
                @switch($bold)
                    @case(1)
                        ...
                        @else
                            @php
                                $nilai = DB::select(
                                    "SELECT (select isnull(sum(nilai),0)nilai from trdrka where jns_ang=? and kd_skpd in($kd_skpd) and (kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd in ($kd_skpd) group by kd_rek6) or left(kd_rek6,4) in ($kode_1)) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd in ($kd_skpd) group by kd_sub_kegiatan) ) anggaran,
                                ...
                                where year(tgl_voucher)='2023' and kd_skpd in($kd_skpd) and (kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd in ($kd_skpd) group by kd_rek6) or left(kd_rek6,4) in ($kode_1)) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd in($kd_skpd) group by kd_sub_kegiatan))
realisasi",
                                    [$jns_ang],
                                );
                                foreach ($nilai as $res) {
                    ...
                    @case(2)
                        @if ($nor == 11)
                            @php
                                ...
                                (select isnull(sum(debet-kredit),0)nilai from trdju_pkd a inner join trhju_pkd b on a.no_voucher=b.no_voucher and a.kd_unit=b.kd_skpd
                                where year(tgl_voucher)='2023' and kd_skpd in($kd_skpd) and kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd in($kd_skpd) group by kd_rek6) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd in($kd_skpd) group by kd_sub_kegiatan))
realisasi",
                                    [$jns_ang],
                                );
                                foreach ($nilai as $res) {
                    ...
                    @case(3)
                        @if ($nor == 12 || $nor == 13 || $nor == 14)
                            @php
                                $nilai = DB::select(
                                    "SELECT (select isnull(sum(nilai),0)nilai from trdrka where jns_ang=? and kd_skpd in($kd_skpd) and kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd=$kd_skpd group by kd_rek6) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd=$kd_skpd group by kd_sub_kegiatan) ) anggaran,
                                (select isnull(sum(debet-kredit),0)nilai from trdju_pkd a inner join trhju_pkd b on a.no_voucher=b.no_voucher and a.kd_unit=b.kd_skpd
                                where year(tgl_voucher)='2023' and kd_skpd in($kd_skpd) and kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd=$kd_skpd group by kd_rek6) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd=$kd_skpd group by kd_sub_kegiatan))
realisasi",
                                    [$jns_ang],
                                );
                                foreach ($nilai as $res) {
                            ...
                            @php
                                ...
                                select kd_skpd,kd_sub_kegiatan,kd_rek6,sum(nilai) anggaran,0 realisasi from trdrka where jns_ang=? and kd_skpd in($kd_skpd) and kd_rek6 in (select kd_rek6 from map_kes_mandatory where kd_skpd in($kd_skpd) group by kd_rek6) and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory where kd_skpd in($kd_skpd) group by kd_sub_kegiatan)
            ...
            @php
                $no2 = DB::select(
                    "SELECT
                        (select isnull(sum(nilai),0)nilai from trdrka where jns_ang=? and kd_skpd in(select kd_skpd from map_kes_mandatory group by kd_skpd) and
                        (kd_rek6 in (select kd_rek6 from map_kes_mandatory  group by kd_rek6) or left(kd_rek6,4)in('5101','5102','5105','5106','5201','5202','5203','5204','5205','5206','5402'))
                        and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory group by kd_sub_kegiatan))
anggaran,
                        (select isnull(sum(debet-kredit),0)nilai from trdju_pkd a inner join trhju_pkd b on a.no_voucher=b.no_voucher and a.kd_unit=b.kd_skpd
                        where year(tgl_voucher)='2023' and kd_skpd in(select kd_skpd from map_kes_mandatory group by kd_skpd) and
                        (kd_rek6 in (select kd_rek6 from map_kes_mandatory  group by kd_rek6) or left(kd_rek6,4)in('5101','5102','5105','5106','5201','5202','5203','5204','5205','5206','5402'))
                        and kd_sub_kegiatan in (select kd_sub_kegiatan from map_kes_mandatory group by kd_sub_kegiatan) ) realisasi",
                    [$jns_ang],
                );
