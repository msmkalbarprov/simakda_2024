...
@section('title', 'CALK EDIT BAB III KETERANGAN NERACA | SIMAKDA')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    Edit Uraian Bab III Rinci NERACA
...
@include('akuntansi.cetakan.calk.bab3.neraca.modal_edit_bab3_neraca_edit_tambah')
@endsection
@section('js')
    <script>
    $(document).ready(function() {
        ...
        $(".select_").select2({
            theme: 'bootstrap-5',
            dropdownParent: $('#modal_edit_bab3_neraca_edit_tambah .modal-content'),
        ...
        let data=$('#table_edit').DataTable({
            ...
            lengthMenu: [10, 20],
            ajax: {
                "url": "{{ route('calk.load_calkbab3_neraca_edit_tambah') }}",
                "type": "POST",
    ...
    $('#tambah').on('click', function() {
        ...
        $('#nilai').val(null);
        // tampil_nilai("");
        $('#modal_edit_bab3_neraca_edit_tambah').modal('show');
    });
    ...
    function hapus(kd_skpd, nm_skpd, kd_rek, nm_rek, ket, nilai) {
        ...
        if  (del==true){
            $(document).ready(function(){
                $.ajax({
                    type     : "POST",
                    url      : "{{ route('calk.hapus_calkbab3_neraca_edit_tambah') }}",
                    data     : ({tabel:'isi_neraca_calk',kd_skpd:kd_skpd,kd_rek:kd_rek,ket:ket,nilai:nilai}),
                    dataType : "json",
    ...
    function simpan(){
        ...
        $(document).ready(function(){
            $.ajax({
                type: "POST",
                url: "{{ route('calk.simpan_calkbab3_neraca_edit_tambah') }}",
                data: ({tabel:'isi_neraca_calk',kd_skpd:kd_skpd,kd_rek:kd_rek,ket:ket,nilai:nilai}),
                dataType:"json",
                success  : function(data){
                    status = data;
